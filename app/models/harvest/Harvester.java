...
import models.harvest.biocase.ResponseToFileHandler;
...
public class Harvester {
	 ...
	 * @throws Exception 
	 */
	public Harvester(Dataset dataset, String targetDirectory, String begin, String end) {
		this.dataset = dataset;
		if (this.dataset.tempDirectory != null) {
			this.targetDirectory = dataset.tempDirectory;
		} else {
			this.targetDirectory = targetDirectory + File.separator + "resource-" + dataset.id + File.separator;
		}
	...
	private void fullHarvest() throws Exception {
		...
		if (!new File(this.dataset.tempDirectory + "0_null-AAA-0_.txt.gz").exists() 
				|| new File(this.dataset.tempDirectory + "zzz-null-0.txt.gz").exists()) {
			...
			// else if the dataset has not been completely harvested the last time, continue from the breaking point
			...
			for (char c1='A'; c1 <= 'Z'; c1++) {
				for (char c2 = 'a'; c2 <= 'z'; c2++) {
					 ...
					 if (!(new File(this.dataset.tempDirectory + lower + "-" + upper + "-0.txt.gz").exists()) 
						...
						for (int startAt = 0; new File(this.dataset.tempDirectory + lower+"-"+upper+"-"+startAt+"_.txt.gz").exists(); startAt = startAt + this.maxResults) {
	 ...
	 */
	public void pageRange(String lower, String upper, int startAt) throws Exception {
		...
		String responseFile = targetDirectory + ((lower == null)? "0_" + lower : lower) + "-" + upper + "-" + startAt +".txt.gz";
		...
		if (this.dataset.type.equals("biocase")) {
			...
			models.harvest.digir.ResponseToFileHandler responseToFile = new models.harvest.digir.ResponseToFileHandler(responseFile);
			httpClient.execute(httpget, responseToFile);
		} else if (this.dataset.type.equals("tapir14")) {
			models.harvest.tapir14.ResponseToFileHandler responseToFile = new models.harvest.tapir14.ResponseToFileHandler(responseFile);
			httpClient.execute(httpget, responseToFile);
		} else if (this.dataset.type.equals("tapir2011")) {
			models.harvest.tapir2011.ResponseToFileHandler responseToFile = new models.harvest.tapir2011.ResponseToFileHandler(responseFile);
			httpClient.execute(httpget, responseToFile);
		}
		...
		try {
			...
			if (this.dataset.type.equals("biocase")) {
				...
				models.harvest.tapir14.ResponseToModelHandler modelFactory = new models.harvest.tapir14.ResponseToModelHandler();
				occurrences = modelFactory.handleResponse(contentStream);
			} else if (this.dataset.type.equals("tapir2011")) {
				models.harvest.tapir2011.ResponseToModelHandler modelFactory = new models.harvest.tapir2011.ResponseToModelHandler();
				occurrences = modelFactory.handleResponse(contentStream);
			}
		} finally {
			...
			if ( path.exists() ) {
				...
				for (int i = 0; i < files.length; ++i) {
					if (files[i].isDirectory()) {
						deleteTemporaryDirectory(files[i].getAbsolutePath(), lower);
					} else if (lower != null) {
						if ((files[i].toString().split(directory)[1].split("-")[0]).compareTo(lower)>= 0 
							&& !(files[i].toString().substring(files[i].toString().length()- 10, files[i].toString().length()).equals("-0_.txt.gz"))) {
							files[i].delete();
						}
					} else {
						files[i].delete();
	...
	public void deleteOccurrencesFrom(Dataset dataset, String lower) {
		...
		LOG.info("Occurrences upper than " + lower +" are deleted from the database");
	}
}